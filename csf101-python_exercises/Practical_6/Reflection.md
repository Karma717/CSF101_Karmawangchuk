...
Linear Search Sequentially checks each element in the list until the target is found or the list ends. Works on unsorted lists. Time complexity: O(n).
Binary Search (Iterative) Works only on sorted lists. Repeatedly divides the search range in half and compares the middle element to the target. Time complexity: O(log n).
Binary Search (Recursive) Same logic as iterative binary search but implemented using recursion. Recursively searches either the left or right half of the array. Time complexity: O(log n), with extra space due to recursive call stack.

Reflection
"I now understand the different ways to search for things in a list:
Search Techniques Linear search is easy to do, but it's really slow when you have a lot of data.
...
Key Takeaways: The most important things I learned are:
