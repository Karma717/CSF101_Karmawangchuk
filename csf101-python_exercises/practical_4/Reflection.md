...
Challenges Faced
1 Infinite Loop When Reversing: I got stuck because I forgot the most important step: telling the list to move to the next node inside my loop (current = next_node).

I fixed it by making sure the pointer that travels through the list was always correctly updated.

Deleting the Wrong Item: When trying to remove the Nth item from the end, I mixed up the starting distance (offset) between my fast and slow pointers.
...
Merging Empty Lists: My list merging code broke when one of the lists was empty.
